...
documents.post("/query/", authenticated, async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, message: "Query executed", data: { documents: result.rows } }, 200);
  } catch (error) {
    console.error(error);
...
documents.put("/update/:id", authenticated, async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, message: "Document updated", data: { document: updatedDocument.rows[0] } }, 200);
  } catch (error) {
    console.error(error);
...
documents.get("/id/:id", authenticated, async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, message: "Document found", data: { document: document.rows[0] } }, 200);
  } catch (error) {
    console.error(error);
...
documents.post("/create/:ext", authenticated, async (c) => {
  ...
  try {
    document = await extension?.hooks?.onCreate?.(document);
  } catch (error) {
  ...
  try {
    ...
    return c.json({ success: true, message: "Document created", data: { document: createdDocument.rows[0] } }, 201);
  } catch (error) {
    console.error(error);
    return c.json({ success: false, message: "Failed to create document" }, 500);
  }
